...
function Get-Csv(){
    <#
        .SYNOPSIS
            Basic function that imports a CSV but will write a LogMessage should this fail.
        .EXAMPLE
            Get-Cred -CredLocation C:\Scripts\Credential.xml

            This will load the credential previously saved in 'Credential.xml' at 'c:\scripts'.
        .DESCRIPTION
             Basic function that imports a CSV but will write a LogMessage should this fail.
             This Logmessage will be of the 'Critical' severity and cause your script to exit afterwards.            
                    
        .PARAMETER CSVLocation
            Location where to read and/or write the CSV file
    ...
    try{
        return Import-Csv $CSVLocation
    }
    catch{
        Write-LogMessage -Severity "Critical" -LogMessage "Reading the CSV File failed from location: $CSVLocation with error $($error[0])"
    }
}
...
function Get-Cred(){
    <#
        .SYNOPSIS
            Fetches a previously saved credential into a PS Credential object.
        .EXAMPLE
            Get-Cred -CredLocation C:\Scripts\Credential.xml

            This will load the credential previously saved in 'Credential.xml' at 'c:\scripts'.
        .DESCRIPTION
            The function will first test whether or not the file is present on the specified location.
            Should the file not be present, the user will be prompted to input a credential after which it is saved for future use.
            A prompt for credential will not appear when running in 'non-interactive' mode (via Task Scheduler, ...).

            After the check we'll load the credential and return an object of the type PS Credential.
            Export-CliXml and Import-CliXML are used, please refer to their respective documentation for specifics.
            Due to their specific nature the generation of a new XML file is needed per user and/or machine.
            
                    
        .PARAMETER CredLocation
            Location where to read and/or write the XML file
    ...
    if (Test-Path $CredLocation){
        ###File exists , no action needed
        Write-LogMessage -Severity "Log" -LogMessage "Credentials found."
    }else{
        ###write log to prompt for password
        Write-LogMessage -Severity "WARNING" -LogMessage "Credential file not found. Prompting for Credentials."
        Write-LogMessage -Severity "WARNING" -LogMessage "Prompt will not appear when running in task scheduler."
        $cred = Get-Credential
        try{
            $cred | Export-Clixml $CredLocation
        }catch{
            Write-LogMessage -Severity "Critical" -LogMessage "Saving credentials failed with error: $($error[0])"    
        }
    }


    try{
        $credential = Import-Clixml $CredLocation
        return $Credential
    }catch{
